        ...
        @foreach($orders as $item)
            @if ($loop->first)
                <div class="col-span-1 md:col-span-3 p-6 flex justify-start items-center">
                    <img src="{{$item->user->getavatar()}}" width="50px" height="auto">
                    <h3 class="text-gray-900 dark:text-white" style="margin-right: 6px">{{$item->user->name}}</h3>
                </div>
            @endif
            
            @if (!$loop->first && $orders[$loop->index -1]->user->name != $item->user->name )
            <div class="col-span-1 md:col-span-3 p-6 flex justify-start items-center">
                <img src="{{$item->user->getavatar()}}" width="50px" height="auto">
                <h3 class="text-gray-900 dark:text-white" style="margin-right: 6px">{{$item->user->name}}</h3>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        @foreach($orders as $item)
            @if ($loop->first)
                <div class="col-span-1 md:col-span-3 p-6 flex justify-start items-center">
                    <img src="{{$item->user->getavatar()}}" width="50px" height="auto">
                    <h3 style="margin-right: 6px">{{$item->user->name}}</h3>
                </div>
            @endif
